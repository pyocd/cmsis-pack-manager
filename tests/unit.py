"""Unit tests for the arm_pack_manager module"""
...
import arm_pack_manager

@given(booleans(), booleans(), text(), text())
@example(True, True, '', '')
@example(True, True, '/', '/')
def test_init(silent, no_timeouts, json_path, data_path):
    obj = arm_pack_manager.Cache(silent, no_timeouts, json_path=json_path, data_path=data_path)
...
def test_largest_version(version_strings, max_version):
    newest = arm_pack_manager.largest_version(version_strings)
    if max_version:
...
def test_do_queue(queue):
    to_run = MagicMock()
    arm_pack_manager.do_queue(arm_pack_manager.Reader, to_run, queue)
    for blah in queue:
...
def test_strip_protocol(protocol, url):
    uri = protocol + "://" + url
    assert(arm_pack_manager.strip_protocol(uri) == url)

@given(text(alphabet=ascii_lowercase + ":/_."), text())
def test_cache_file(url, contents):
    @patch("arm_pack_manager.Cache._cache_lookup")
    @patch("arm_pack_manager.Cache.display_counter")
    @patch("arm_pack_manager.urlopen")
    @patch("arm_pack_manager.makedirs")
    @patch("arm_pack_manager.open", create=True)
    def inner_test(_open, _, _urlopen, __, _cache_lookup):
        ...
        c = arm_pack_manager.Cache(True, True)
        c.cache_file(url)
...
def test_pdsc_from_cache(data_path, url):
    @patch("arm_pack_manager.BeautifulSoup")
    @patch("arm_pack_manager.open", create=True)
    def inner_test(_open, _bs):
        _open.return_value.__enter__.return_value = MagicMock
        c = arm_pack_manager.Cache(True, True, data_path=data_path)
...
def test_pack_from_cache(data_path, url):
    @patch("arm_pack_manager.ZipFile")
    def inner_test(_zf):
        c = arm_pack_manager.Cache(True, True, data_path=data_path)
...
def test_get_pack_url(name, vendor, url, versions):
    xml = Template(VERSION_TEMPLATE).render(name=name, vendor=vendor, url=url,
                                            versions=versions)
    @patch("arm_pack_manager.largest_version")
...
def test_pdsc_to_pack(url):
    @patch("arm_pack_manager.Cache.get_pack_url")
    @patch("arm_pack_manager.Cache.pdsc_from_cache")
...
def test_cache_pdsc_and_pack(pack_url, pdsc_url):
    @patch("arm_pack_manager.Cache.cache_file")
    @patch("arm_pack_manager.Cache.pdsc_to_pack")
