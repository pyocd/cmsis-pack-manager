    ...
    fn build(self, debugs: &Vec<Debug>) -> Result<Vec<Processor>, Error> {
        ...
        let map = (0..units)
            .map(|unit| {
                ...
                let debugs = debugs.iter().filter(|debug| {
                ...
                Ok(Processor {
                    ...
                    dp: debugs.clone().find_map(|d| d.dp).unwrap_or_default(),
                    ap: debugs.clone().find_map(|d| d.ap).unwrap_or_default(),
                    address: debugs.clone().find_map(|d| d.address),
                    svd: debugs.clone().find_map(|d| d.svd.clone()),
                    name: name.clone(),
                    unit,
                    default_reset_sequence: debugs
                        .clone()
